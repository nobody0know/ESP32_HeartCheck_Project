                    ...
                    if (usermsg_send_start)
                    {
                        if (chan_num < SOC_ADC_CHANNEL_NUM(ECG_ADC_UNIT))
                        {
                            ...
                            if (i % 10 == 0)
                            {
                                ...
                                adc_true_value.payload_data.timestamp = adc_data_timebase + time_flag_gap + time_insert;
                                time_insert++; // 时间插值，将数据的时间戳插值
                                adc_value_avg = 0;
                                xQueueSend(ADC_queue, &adc_true_value, 1);
                                read_data_num ++;
                            }
