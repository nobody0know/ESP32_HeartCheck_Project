...
esp_err_t wifi_connect_config_reinit()
{
    ...
    memcpy(wifi_config.sta.password, password, strlen((char *)password));

    ESP_LOGI(TAG, "connect WIFI ssid:%s and password:%s", wifi_config.sta.ssid, wifi_config.sta.password);
    ...
    ESP_LOGI(TAG, "wifi reinit config finished.");
    return ESP_OK;
}
